...
export default function Category() {
    ...
    useEffect(() => {
        // Fetch the initial listings when the component mounts or when the category name changes
        async function fetchListings() {
            try {
                // Get reference to the 'listings' collection
                const listingRef = collection(db, "listings");

                // Create the query to get listings of the specific category, sorted by timestamp in descending order and limited to 8 listings
                const q = query(
                    listingRef,
    ...
    async function onFetchMoreListings() {
        try {
            ...
            // Create the query to get more listings of the specific category, sorted by timestamp in descending order and limited to 4 listings after the last fetched listing
            const q = query(
                listingRef,
                where("type", "==", params.categoryName),
                orderBy("timestamp", "desc"),
    ...
    return (
        <div className="max-w-6xl mx-auto px-3">
            {/* Render the category name in the heading */}
            ...
            {loading ? (
                ...
                <p>There are no current {params.categoryName === "rent" ? "Places for rent" : "Places for sale"}</p> // Show a message if there are no listings
            )}
        </div>
    );
}
