        ...
        <div className='w-full md:w-[67%] lg:w-[40%] lg:ml-20'>
          ...
          <form onSubmit={onSubmit}>
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} id='password' value={password} onChange={onChange} placeholder='Password' className='w-full mb-6 px-4 py-2 text-xl text-gray-700 bg-white border-gray-300 rounded transition ease-in-out' />
              {showPassword ? (<AiFillEyeInvisible className='absolute right-3 top-3 text-xl cursor-pointer' onClick={() => setShowPassword((prevStatess) => !prevStatess)} />)
                : (<AiFillEye className='absolute right-3 top-3 text-xl cursor-pointer' onClick={() => setShowPassword((prevStatess) => !prevStatess)} />)}
            </div>
            <div className="flex justify-between whitespace-nowrap text-sm sm:text-lg">
              <p className='mb-6'>Don't have a account?
                <Link to="/sign-up" className='ml-1 text-red-600 hover:text-red-700 transition duration-200 ease-in-out'>Register</Link></p>
            ...
            <button type='submit' className='w-full bg-blue-600 text-white px-7 py-3 text-sm
             font-medium uppercase rounded shadow-md hover:bg-blue-700 transition duration-200 ease-in-out
